                    ...
                    <div className="flex flex-col w-full">
                        ...
                        <div className="flex flex-row w-full items-center justify-between">
                            <div className="flex flex-row items-center">
                                <span>Patch: </span>
                                {this.props.patches.map((v, i) => {
                                    return <button key={i} className={`h-8 w-8 mx-2 cursor-pointer outline-none rounded-lg ${this.props.activePatch === i ? "bg-green-700 hover:bg-green-600 active:bg-green-800" : "bg-slate-700 hover:bg-slate-600 active:bg-slate-800"} border-[1px] border-transparent active:border-slate-700 shadow-md active:shadow-inner flex justify-center items-center duration-75`} title={v.name} onClick={() => this.props.changePatch(i)}>{i}</button>
                                })}
                            </div>
                            <div className="flex flex-row items-center">
