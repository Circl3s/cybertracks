function Step(props) {
    ...
    return (
        <div className={`m-1 grid grid-cols-4 text-[2.5vh] rounded-lg text-slate-100 font-['VT323'] border-slate-800 ${props.beat ? "border-y-2" : ""} ${props.focused ? "bg-slate-700" : props.active ? "bg-slate-800" : ""}`} onClick={props.onClick}>
            <span className="mx-4 text-slate-200 whitespace-nowrap">{note.padEnd(2, "_")}</span>
            <span className="mx-4 text-red-400 whitespace-nowrap">{octave}</span>
            <span className="mx-4 text-green-400 whitespace-nowrap">{velocity.toString()[0]}</span>
            <span className="mx-4 text-blue-400 whitespace-nowrap">{duration}</span>
        </div>
    );
}
