function DrumStep(props) {
    ...
    const crash = props.step[5] ? "#" : "-"
return (
    <div className={`m-1 grid grid-cols-6 text-[2.5vh] rounded-lg text-slate-100 font-['VT323'] border-slate-800 ${props.beat ? "border-y-2" : ""} ${props.focused ? "bg-slate-700" : props.active ? "bg-slate-800" : ""}`} onClick={props.onClick}>
        ...
        <span className="mx-4 text-slate-200">{crash}</span>
    </div>
);
}
